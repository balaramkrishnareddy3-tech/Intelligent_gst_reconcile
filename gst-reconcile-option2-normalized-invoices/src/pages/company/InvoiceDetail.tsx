...
export default function InvoiceDetail() {
  ...
  const { getInvoiceById } = useGstStore();
    ...
    return (
      <div className="py-16 text-center space-y-4">
        ...
        <p className="text-sm text-slate-500">
          The requested identifier <code className="font-mono text-sky-600">{id}</code> could not be
          located in the centralized reconciliation store.
        </p>
        <Button onClick={() => navigate('/company/reconciliation/results')}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Invoice Investigation: ${invoice.invoiceNumber}`}
        subtitle={`Audit breakdown & field-by-field verification for ${invoice.vendorName}`}
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              size="sm"
              onClick={() => navigate(`/company/transaction-graph?focus=${invoice.invoiceNumber}`)}
            >
              <Network className="h-3.5 w-3.5" /> View in Transaction Graph
            </Button>
          </div>
        }
      />
      ...
      <GlassCard className="p-4 bg-gradient-to-r from-sky-50/70 via-white to-blue-50/70">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 text-xs">
          ...
          <div>
            <span className="text-[10px] text-slate-400 uppercase font-semibold flex items-center gap-1">
              <Building className="h-3 w-3" /> Supplier / Vendor
            </span>
          ...
          <div className="text-right sm:text-left">
            <span className="text-[10px] text-slate-400 uppercase font-semibold">Status & Risk</span>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <GlassCard className="space-y-4 border-l-4 border-l-sky-500">
          <div className="flex items-center justify-between border-b border-sky-100 pb-3">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Building className="h-4 w-4 text-sky-600" /> Company Purchase Record (ERP / Ledger)
            </h3>
          ...
          <div className="space-y-2.5 text-xs">
            ...
            <div className="flex justify-between py-1.5">
              <span className="text-slate-500">ITC Eligibility Status</span>
              <span className="font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                {companyRecord.itcEligible ? 'Eligible as per Books' : 'Ineligible'}
              </span>
            </div>
          </div>
        </GlassCard>
        ...
        >
          <div className="flex items-center justify-between border-b border-sky-100 pb-3">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <FileText className="h-4 w-4 text-indigo-600" /> GST Portal Record (GSTR-2B Auto-Drafted)
            </h3>
            <span
      ...
      <GlassCard className="space-y-4">
        <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2 border-b border-sky-100 pb-3">
          <CheckCircle2 className="h-4 w-4 text-sky-600" /> Field-by-Field Reconciliation Matrix
        </h3>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <GlassCard className="space-y-3 bg-amber-50/40 border-amber-200">
          <h4 className="text-xs font-bold uppercase tracking-wider text-amber-900 flex items-center gap-1.5">
            <AlertTriangle className="h-4 w-4 text-amber-600" /> Mismatch Explanation
          </h4>
          <div className="rounded-xl bg-white p-3.5 border border-amber-200/80 text-xs text-slate-700 space-y-2">
            <p className="font-semibold text-slate-900">
              {invoice.mismatchExplanation || 'No discrepancy detected for this invoice record.'}
            </p>
            {invoice.taxableDifference > 0 && (
              <div className="text-[11px] font-mono text-slate-600 bg-slate-50 p-2 rounded">
                <div>Company Taxable: ₹{companyRecord.taxableValue.toLocaleString('en-IN')}</div>
                <div>
                  GST Record Taxable:{' '}
                  {gstRecord ? `₹${gstRecord.taxableValue.toLocaleString('en-IN')}` : '₹0'}
                </div>
                <div className="font-bold text-rose-600">
                  Taxable Difference: ₹{invoice.taxableDifference.toLocaleString('en-IN')}
                </div>
              </div>
            )}
          </div>
        ...
        <GlassCard className="space-y-3 bg-sky-50/40 border-sky-200">
          <h4 className="text-xs font-bold uppercase tracking-wider text-sky-900 flex items-center gap-1.5">
            <Info className="h-4 w-4 text-sky-600" /> Statutory Risk & Recommended Action
          </h4>
      ...
      <GlassCard className="p-4">
        <h4 className="text-xs font-bold text-slate-700 mb-3 uppercase tracking-wider">
          Explore Connected Audit & Risk Artifacts
        </h4>
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <ShieldAlert className="h-3.5 w-3.5 text-rose-600" /> View ITC Risk Profile
          </Button>

          <Button
            size="sm"
            variant="secondary"
            onClick={() => navigate(`/company/vendor-risk/${invoice.vendorId}`)}
          >
            <Users className="h-3.5 w-3.5 text-sky-600" /> View Vendor Compliance Profile
          </Button>

          <Button
            size="sm"
            variant="secondary"
            onClick={() => navigate('/company/audit-trail')}
          >
            <ClipboardList className="h-3.5 w-3.5 text-indigo-600" /> View Audit Trail History
          </Button>
        </div>
      </GlassCard>
    </div>
